/*
 * Список тем внутри одного раздела форума (?name=...) — промежуточный
 * уровень между каталогом разделов (forum.html) и одной темой
 * (forum-topic.html). Раздел определяется просто текстом в поле
 * forum_topics.section — никакого отдельного справочника разделов нет.
 */
(function () {
  ...
  function loadTopics() {
    window.supa.from('forum_topics')
      .select('id, title, created_at, profiles!author_id(id, nickname, verified)')
      .eq('section', sectionName)
      .order('created_at', { ascending: false })
      .then(function (res) {
        if (res.error || !res.data) {
          topicsBody.innerHTML = '<tr><td colspan="4" class="hint" style="padding:8px">Не удалось загрузить.</td></tr>';
          return;
        }
        if (sbTopicCount) sbTopicCount.textContent = res.data.length;
        if (!res.data.length) {
          topicsBody.innerHTML = '<tr><td colspan="4" class="hint" style="padding:8px">Пока ни одной темы — создайте первую.</td></tr>';
          return;
        }
  ...
  function renderRow(t) {
    ...
    tr.innerHTML =
      '<td class="row2 ic"><img src="img/icons/i-feed.svg" alt=""></td>' +
      '<td class="row1"><a class="ttl" href="forum-topic.html?id=' + t.id + '">' + escapeHtml(t.title) + '</a></td>' +
  ...
  if (newTopicSubmit) {
    newTopicSubmit.addEventListener('click', function () {
      ...
      window.supa.auth.getSession().then(function (res) {
        ...
        window.supa.from('forum_topics')
          .insert({ section: sectionName, title: title, author_id: session.user.id })
          .select('id')
          .single()
          .then(function (topicRes) {
            if (topicRes.error) { setHint(topicRes.error.message, false); newTopicSubmit.disabled = false; return; }
            ...
            window.supa.from('forum_replies')
              ...
              .then(function (replyRes) {
                if (replyRes.error) { setHint(replyRes.error.message, false); newTopicSubmit.disabled = false; return; }
